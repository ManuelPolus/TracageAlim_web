    ...
    public class Mapper
    {
        ...
        public void CreateTreatment(Treatment newTreatment)
        {
            ...
            using (context = new TrackingDataContext("FTDb"))
            {
                try
                {
                    Role rl = context.Roles.FirstOrDefault(r => r.Name == "Administrator" || r.Id == 1);

                    context.Treatements.Add(newTreatment);
                    context.SaveChanges();
                    UserScanRights usr = new UserScanRights { RoleId = rl.Id, TreatmentId = newTreatment.Id };
                    context.UserScanRights.Add(usr);
                    context.SaveChanges();
                }
                catch (Exception e)
                {
